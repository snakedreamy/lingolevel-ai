// server/routes.ts — all API routes in one file (merged from routes/chat.ts + routes/analyze.ts + routes/serverConfig.ts)
...
export function createApiRouter(args: { provider: Provider; cfg: ServerConfig }): Router {
  ...
  router.post('/chat', async (req, res) => {
    ...
    try {
      ...
      logRequest({ endpoint: 'chat', provider: cfg.provider, model: cfg.chatModel, status: 200, latencyMs: Date.now() - start, fallback: !!result.isFallback })
      res.json({ content: result.content, timestamp: Date.now(), isFallback: !!result.isFallback })
    } catch (error) {
      logRequest({ endpoint: 'chat', provider: cfg.provider, model: cfg.chatModel, status: 500, latencyMs: Date.now() - start, retry: 3, fallback: true, errorMsg: errorMessage(error) })
