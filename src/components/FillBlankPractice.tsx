...
export default function FillBlankPractice(props: FillBlankPracticeProps) {
  ...
  return (
    <div className="fill-blank-shell fill-blank-enter mx-auto flex h-full w-full max-w-6xl min-h-0 overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
      <aside className="hidden w-64 shrink-0 border-r border-zinc-200 bg-zinc-50/70 p-5 dark:border-zinc-800 dark:bg-zinc-950/40 md:flex md:flex-col">
        ...
        <ol className="mt-7 min-h-0 flex-1 space-y-1 overflow-y-auto pr-1">
          {practice.cards.map((entry, index) => {
            ...
            return (
              <li ref={active ? activeStepRef : undefined} key={entry.id} className={`flex items-center gap-3 rounded-lg px-2.5 py-2 text-xs font-semibold transition ${active ? 'bg-white text-zinc-900 shadow-sm dark:bg-zinc-800 dark:text-white' : 'text-zinc-500'}`}>
        ...
        <button type="button" onClick={practice.resetToSetup} className="mt-4 flex h-10 shrink-0 items-center gap-2 border-t border-zinc-200 pt-4 text-xs font-semibold text-zinc-500 hover:text-zinc-800 dark:border-zinc-800 dark:hover:text-zinc-200"><ArrowLeft className="h-3.5 w-3.5" /> 结束本组</button>
      </aside>

      <section className="flex min-w-0 flex-1 flex-col overflow-hidden">
        <div className="flex shrink-0 items-center gap-3 border-b border-zinc-100 px-4 py-3 dark:border-zinc-800 sm:px-7">
        ...
        <div ref={contentRef} className="fill-blank-content min-h-0 flex-1 overflow-y-auto px-5 py-7 sm:px-9 sm:py-10 lg:px-14">
          <div className="mx-auto max-w-3xl">
            {practice.isFallback && practice.currentIndex === 0 && (
              <p className="fill-blank-fallback-notice mb-5 rounded-lg bg-amber-50 px-3 py-2 text-xs leading-5 text-amber-700 dark:bg-amber-950/20 dark:text-amber-300">
            ...
            <div className="fill-blank-prompt mt-7 min-h-32 sm:mt-10 sm:min-h-44">
              ...
              <p className="fill-blank-translation mt-5 text-sm leading-6 text-zinc-500 sm:text-base">{card.translation}</p>
            </div>
        ...
        {!settled && (
          <div className="fill-blank-answer-panel max-h-[60%] shrink-0 overflow-y-auto border-t border-zinc-200 bg-white px-5 py-3 pb-[calc(.75rem+env(safe-area-inset-bottom))] dark:border-zinc-800 dark:bg-zinc-900 sm:px-9">
            <div className="mx-auto max-w-3xl">
              ...
              {answerError && (
                <div role="alert" className="mt-2 flex items-start gap-2 text-xs leading-5 text-rose-600 dark:text-rose-400">
              ...
              {showHint && <p className="mt-2 rounded-xl bg-amber-50 px-4 py-2.5 text-sm text-amber-800 dark:bg-amber-950/25 dark:text-amber-200">{card.hint} · {card.answer.length} 个字母</p>}
            </div>
          </div>
        )}
